            ...
            <ul class="nav navbar-top-links">
                ...
                <li class="dropdown">
                    <a href="#" data-toggle="dropdown" class="dropdown-toggle" aria-expanded="true">
                        ...
                        @if($orders > 0 || $sellers > 0 || $review_count > 0||count($conversation) > 0 ||$support_ticket > 0|| count(\App\RefundRequest::where('admin_seen',0)->get()) > 0)
                            ...
                            <div class="nano-content" tabindex="0" style="right: -17px;">
                                <ul class="head-list">
                                    ...
                                    @endif
                                     @if($review_count > 0)
                                        <li>
                                            <a class="media" href="{{ route('reviews.index') }}">
                                                <div class="media-body">
                                                    <p class="mar-no text-nowrap text-main text-semibold">{{ $review_count }} {{__('New Reviews(s)')}}</p>
                                                </div>
                                            </a>
                                        </li>
                                    @endif
                                      ...
                                      @if(count($conversation) > 0)
                                        <li>
                                            <a class="media" href="{{ route('conversations.admin_index') }}">
                                                <div class="media-body">
                                                    <p class="mar-no text-nowrap text-main text-semibold">{{ count($conversation) }} {{__('New Conversation(s)')}}</p>
                                                </div>
                                            </a>
                                        </li>
                                    @endif
                                      @if($support_ticket > 0)
                                        <li>
                                            <a class="media" href="{{ route('support_ticket.admin_index') }}">
                                                <div class="media-body">
                                                    <p class="mar-no text-nowrap text-main text-semibold">{{ $support_ticket }} {{__('New Support Ticket Notification(s)')}}</p>
                                                </div>
                                            </a>
                                        </li>
                                    @endif
                                    ...
                                    @if (\App\Addon::where('unique_identifier', 'refund_request')->first() != null)
                                      @if(count(\App\RefundRequest::where('admin_seen',0)->get()) > 0)
                                        <li>
                                            <a class="media" href="{{ route('support_ticket.admin_index') }}">
                                                <div class="media-body">
                                                    <p class="mar-no text-nowrap text-main text-semibold">{{ count(\App\RefundRequest::where('admin_seen',0)->get()) }} {{__('New Refund Notification(s)')}}</p>
                                                </div>
                                            </a>
                                        </li>
                                    @endif
