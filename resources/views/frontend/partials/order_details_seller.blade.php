...
<div class="modal-body gry-bg px-3 pt-0">
    <div class="pt-4">
        <ul class="process-steps clearfix">
            ...
            <li
                @if ($status == 'ready_to_ship') class="active" @elseif($status == 'shipped' || $status == 'delivered') class="done" @endif>
                <div class="icon">2</div>
                <div class="title">{{ __('Ready to ship') }}</div>
            </li>
            <li
                @if ($status == 'shipped') class="active" @elseif($status == 'delivered') class="done" @endif>
        ...
        <div class="offset-lg-2 col-lg-8 col-sm-12">
            <div class="form-inline">
                <select class="form-control selectpicker form-control-sm" data-minimum-results-for-search="Infinity"
                    ...
                    <option value="pending" @if ($status == 'pending') selected @endif>{{ __('Pending') }}
                    </option>
                    <option value="ready_to_ship" @if ($status == 'ready_to_ship') selected @endif>{{ __('Ready to ship') }}
                    </option>
                    <option value="shipped" @if ($status == 'shipped') selected @endif>
                        {{ __('Shipped') }}</option>
                    <option value="delivered" @if ($status == 'delivered') selected @endif>{{ __('Delivered') }}
                    </option>
                    ...
                    <option value="failed_delivery" @if ($status == 'failed_delivery') selected @endif>{{ __('Failed Delivery') }}</option>
                    <option value="refund_only" @if ($status == 'refund_only') selected @endif>{{ __('Refund Only') }}</option>
                </select>
                    ...
                    <table class="details-table table">
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                {{ json_decode($order->shipping_address)->district ?? '' }},
                                {{ json_decode($order->shipping_address)->division ?? '' }},
    ...
    <div class="row">
        <div class="col-lg-9">
            <div class="card mt-4">
                ...
                <div class="card-body pb-0" style="overflow:auto;">
                    <table class="details-table table">
                        <thead>
                            <tr>
                                <th>#</th>
                                        ...
                                        <td>
                                            @if ($orderDetail->product != null && $orderDetail->product->refundable != 0 && $orderDetail->refund_request == null)
                                                <button type="submit" class="btn btn-styled btn-sm btn-base-1" 
                                                   onclick="window.location.href = '{{ route('refund_request_send', $orderDetail->id) }}'">{{ __('Send') }}</button>
