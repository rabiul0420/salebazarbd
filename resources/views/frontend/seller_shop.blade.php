...
@section('content')
                ...
                <div class="col-md-6">
                    <div class="d-flex">
                        <img
                            height="70"
                            class="lazyload"
                            src="{{ !empty($shop->logo) ? asset($shop->logo) : asset('frontend/images/placeholder.jpg') }}"
    ...
    @if (!isset($type))
        @if ($shop->is_banner_hidden != null && $shop->is_banner_hidden == 0)
        <section class="py-4">
            <div class="container">
                    ...
                    <div class="col">
                        <div class="caorusel-box arrow-round gutters-15">
                            <div class="slick-carousel center-mode" data-slick-items="5" data-slick-lg-items="3"  data-slick-md-items="3" data-slick-sm-items="1" data-slick-xs-items="1">
                                @foreach ($shop->user->products->where('published', 1)->where('featured', 1)->where('is_approved', 1) as $key => $product)
    ...
    <section class="@if (!isset($type)) gry-bg @endif pt-5">
        <div class="container">
            ...
            <div class="product-list row gutters-5 sm-no-gutters">
                @php
                    if (!isset($type)){
                        $products = \App\Product::where('user_id', $shop->user->id)->where('published', 1)->orderBy('created_at', 'desc')->where('is_approved', 1)->paginate(24);
                    }
                    elseif ($type == 'top_selling'){
                        $products = \App\Product::where('user_id', $shop->user->id)->where('published', 1)->orderBy('num_of_sale', 'desc')->where('is_approved', 1)->paginate(24);
                    }
                    elseif ($type == 'all_products'){
                        $products = \App\Product::where('user_id', $shop->user->id)->where('published', 1)->where('is_approved', 1)->paginate(24);
                    }
                @endphp
