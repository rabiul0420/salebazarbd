    			...
    			<div class="col-sm-6 text-xs-center">
    				<address>
        				<strong class="text-main">{{ json_decode($order->shipping_address)->name }}</strong><br>
        				 ...
        				 {{ json_decode($order->shipping_address)->address }}, {{ json_decode($order->shipping_address)->city }}, {{ json_decode($order->shipping_address)->region ?? '' }}
                    </address>
    			</div>
    			<div class="col-sm-6 text-xs-center">
                    ...
                    <tr>
    					...
    					<td class="text-right">
    						{{ json_decode($order->delivery_type)->shipping_type }}
    					</td>
    				</tr>
                    @if (json_decode($order->delivery_type)->shipping_type == 'Pick-up Point')
                        <tr>
                            <td class="text-main text-bold">{{__('Pickup Point')}}:</td>
                            <td class="text-right">{{ \App\PickupPoint::where('id',json_decode($order->delivery_type)->pickup_point_id)->first()->name }}</td>
                        </tr>
                        <tr>
        					<td class="text-main text-bold">
        					...
        					<td class="text-right">
        						{{ App\PickupPoint::where('id',json_decode($order->delivery_type)->pickup_point_id)->first()->address }}
        					</td>
        				</tr>
                        <tr>
                            <td class="text-main text-bold">{{__('Phone')}}:</td>
                            <td class="text-right">{{ \App\PickupPoint::where('id',json_decode($order->delivery_type)->pickup_point_id)->first()->phone }}</td>
                        </tr>
                    @endif
    				</tbody>
    				</table>
